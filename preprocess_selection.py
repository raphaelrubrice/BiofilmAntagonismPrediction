...
if __name__ == "__main__":
    ...
    parser.add_argument("run", nargs="?", default=0, type=int, help="Run the evaluation.")
    parser.add_argument("concat_results", nargs="?", default=0, type=int, help="Concatenate result files.")
    parser.add_argument("mode", nargs="?", default="ho", type=str, help="Evaluation mode: 'ho' or 'cv'.")
    ...
    if args.run:
        ...
        num_cols = [col for col in combinatoric_df.columns if col not in cat_cols + remove_cols + target]
        ...
        for key, pipeline in pipeline_dict.items():
            results = evaluate(pipeline, key, df_dict, mode=MODE, suffix=PROTOCOL_SUFFIX,
            ...
            results_df.to_csv(f"Results/preprocess_selection/{key}_LGBMRegressor_{MODE}_results.csv", index=False)
    ...
    if args.concat_results:
        ...
        if all_results:
            ...
            all_df.to_csv(os.path.join(results_folder, "ho_all_results.csv"), index=False)
        else:
